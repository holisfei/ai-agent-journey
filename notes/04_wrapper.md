...
通过@wraps(func)来实现原函数的信息保留(from functools import wraps)，可以通过原函数.__wrapped__来获取原函数本身
...
使用@xxx来使用定义好的函数装饰器，本质就是将这个函数作为参数传入到装饰器
...
可以看到无参类装饰器本质就是，用``` __init__``` 和 ```__call__``` 来分别接受函数和参数
